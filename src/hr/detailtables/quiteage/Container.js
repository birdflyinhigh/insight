...
import {TableColum} from '../quite/constant';
import {Table, Pagination} from 'antd';

function mapStateToProps(state){
  return {...state.statetree.quiteageTable};
}
...
export default class Keyposition extends Component{
  ...
  downloadData(){
    CommonMethod.download( "erp/hr/exportQuitNumAgeInfo",this.queryParams);
  }
  render(){
    ...
    return <div className="financial-bg">
      <div className="hr-container">
        ...
        <Link to="/hr" style={{fontSize: 14}}>返回上级页面</Link>
        离职人员司龄分布（人）明细数据({state.name})
        </h1>
