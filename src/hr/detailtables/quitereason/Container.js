...
function mapStateToProps(state){
  return {...state.statetree.quitereasonTable};
}
...
export default class Keyposition extends Component{
  constructor(props){
    ...
    this.queryParams = {
      ...
      reason: this.props.history.location.state ? this.props.history.location.state.name : "",
      pageNum: 1,
      pageSize: 15, 
    };
    this.state = {
  ...
  downloadData(){
    CommonMethod.download( "erp/hr/exportQuitNumReasonInfo",this.queryParams);
  }
  render(){
    ...
    return <div className="financial-bg">
      <div className="hr-container">
        ...
        <Link to="/hr" style={{fontSize: 16, fontWeight: "bold"}}>>>返回</Link>
        离职原因分析明细数据({state.name})
        </h1>
