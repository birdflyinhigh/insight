...
export default class Keyposition extends Component{
  constructor(props){
    ...
    this.queryParams = {
      ...
      orgID: this.props.history.location.state ? this.props.history.location.state.id : "",
      pageNum: 1,
      pageSize: 15
    };
    this.state = {
  ...
  downloadData(){
    CommonMethod.download( "erp/hrcost/exportWorkHours",this.queryParams);
  }
    ...
    return <div className="financial-bg">
      <div className="hr-container">
        ...
        <Link to="/hrmanage" style={{fontSize: 14}}>返回上级页面</Link>
          人均工作时长明细数据({state.name})
        </h1>
        <div className="mdetail-container">
          <p style={{fontSize: 16}}>
            <span style={{paddingLeft: 20}}>数据时间： {state.year}年{state.month}月</span>
            {/* <span style={{paddingLeft: 20}}>排名指标： {state.indexName}</span> */}
            {/* <span style={{paddingLeft: 20}}>排名层级： {state.levelName}</span> */}
            {/* <span style={{paddingLeft: 20}}>当前展示： {state.departName}</span> */}
