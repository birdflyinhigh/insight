...
import SaveDetail from '../component/detail/SaveDetail';
import DetailHeader from '../component/header/detailHeader';
import {Methods} from '../../common/tools/util';
import {CommonMethod} from '../../common/tools/common';
import {saveAction, saveUiAction, xhrActionName} from '../action/action';
...
function mapStateToProps(state){
  ...
  return {...state.statetree.purchase.save};
}
function mapDispatchToProps(dispatch){
  return bindActionCreators(Object.assign({sendRequest: CommonMethod.sendRequest}, saveUiAction), dispatch);
}
...
export default class DetailPage extends Component{
  ...
  componentDidMount() {
   ...
   function selectItem() {
     this.props.sselectType(this.queryParams.organizeType);
   ...
   function getRestData() {
     xhrActionName.save.forEach((actionName, index) => {
       if (index > 7) {
         sendRequest({
  ...
  render(){
    ...
    let mainData = {
      barData: this.props.saveBarData,
      tableData: this.props.firstTableData,
    }
    let mainfn = {
    ...
    return <div className="erp-container">
      <h1 className="erp-pagetitle">
        <Link to="/purchase">>>返回</Link>采购节约金额分析
      </h1>
      <DetailHeader data={headerData} fns={fns}/>
      <SaveDetail data={mainData} fns={mainfn}/>
    </div>
  }
}