...
import OverviewDetail from '../component/detail/OverviewDetail';
import DetailHeader from '../component/header/detailHeader';
import {Methods} from '../../common/tools/util';
import {CommonMethod} from '../../common/tools/common';
import {overviewUiAction, overviewAction, xhrActionName} from '../action/action';
...
function mapStateToProps(state){
  ...
  return {...state.statetree.purchase.overview};
}
function mapDispatchToProps(dispatch){
  return bindActionCreators(Object.assign({sendRequest: CommonMethod.sendRequest}, overviewUiAction), dispatch);
}
...
export default class DetailPage extends Component{
  ...
  componentDidMount() {
    ...
    function selectItem() {
      this.props.oselectType(this.queryParams.organizeType);
    ...
    function getRestData() {
      xhrActionName.overview.forEach((actionName, index) => {
        if (index > 7) {
          sendRequest({
  ...
  render(){
    ...
    return <div className="erp-container">
      <h1 className="erp-pagetitle">
        <Link to="/purchase">>>返回</Link>采购金额月度分析
      </h1>
      <DetailHeader data={headerData} fns={fns}/>
      <OverviewDetail data={mainData} fns={mainfn}/>
    </div>
  }
}